<?php
/**
* @version $Id: membership_ccci.html.php,v 1.1 2005/12/04 04:40:20 aslam Exp $
* @package Mambo
* @subpackage Weblinks
        ...
        */
        function PdfIcon( $row, $params, $hide_js ) {
                ...
                if ( $params->get( 'pdf' ) && !$params->get( 'popup' ) && !$hide_js ) {
                        ...
                        $link = $mosConfig_live_site. '/index2.php?option=com_membership&amp;do_pdf=1&amp;id='. $row->id.'&amp;type_id='. $row->type_id;
                        if ( $params->get( 'icons' ) ) {
        ...
        */
        function showTable( &$params, &$rows, $type_id, $tabclass ) {
                ...
                foreach ($rows as $row) {
                        ...
                        $link = sefRelToAbs( 'index.php?option=com_membership&task=view&Itemid='.$Itemid.'&type_id='. $type_id .'&id='. $row->id );
        ...
        function showMember( &$rows, $type_id, $id, $tabclass, &$params, $currentcat=NULL ) {
                ...
                if ( $params->get( 'item_title' ) || $params->get( 'pdf' )  || $params->get( 'print' ) || $params->get( 'email' ) ) {
                        ...
                        $print_link = $mosConfig_live_site. '/index2.php?option=com_membership&amp;task=view&amp;id='. $row->id .'&amp;Itemid='. $Itemid .'&amp;pop=1';
                        ?>
                ...
                <?php
                        ...
                        $link = sefRelToAbs( 'index.php?option=com_membership&task=view&type_id='. $type_id .'&id='. $row->id );
                        $menuclass = 'category'.$params->get( 'pageclass_sfx' );
                        $txt="";
                        ?>
                        <tr>
        ...
        function showCategories( &$params, &$categories, $type_id ) {
                ...
                foreach ( $categories as $cat ) {
                        if ( $type_id == $cat->type_id ) {
                                ...
                                $link = 'index.php?option=com_membership&Itemid=' . $Itemid. '&type_id='. $cat->type_id;
                                ?>
                                <li>
