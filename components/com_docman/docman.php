...
* @version $Id: docman.php,v 1.41 2007/03/01 08:33:01 aslam Exp $
...
function doc_purchase_step2_member($pid)
{
    ...
    $lists = array();
    $lists['type_id']                      = mosAdminMenus::MemberType( 'type_id','' );
...
function doc_purchase_step3($pid,$customer)
{
    ...
    elseif (strtolower(trim($customer))=="mem")
    {
       if(strtolower(trim($mosConfig_owner))=="bkmea"){
             $sql="select m.*, mh.member_reg_no from #__member as m"
                ."\n join #__member_history as mh on m.id=mh.member_id"
                ."\n where m.is_delete='0' and mh.member_reg_no='".$_POST['member_reg_no']."'"
                ;
       }else{
            $sub=" and m.type_id='".$_POST['type_id']."'";
            // Check existing member
            $sql="select m.*, mh.member_reg_no from #__member as m"
                ."\n join #__member_history as mh on m.id=mh.member_id"
                ."\n join #__member_type as mt on mt.id=m.type_id"
                ."\n where m.is_delete='0' and mh.member_reg_no='".$_POST['member_reg_no']."'"
                ."\n and mh.reg_year_id='".$_SESSION['working_reg_year_id']."'".$sub
                ;
       }
       ...
       $_SESSION['title']=$result[0]->applicant_title;
       $_SESSION['name']=$result[0]->applicant_name;
       $_SESSION['last_name']=$result[0]->applicant_last_name;
       $_SESSION['firm_name']=$result[0]->firm_name;
       $_SESSION['address']=$result[0]->applicant_address_street;
       $_SESSION['country']=$result[0]->applicant_address_country;
       $_SESSION['phone']=$result[0]->applicant_office_phone;
       $_SESSION['mobile']=$result[0]->applicant_mobile;
       $_SESSION['email']=$result[0]->applicant_email;
       /*
