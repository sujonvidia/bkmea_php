<?php
/**
* @version $Id: user.php,v 1.4 2005/01/06 01:13:28 eddieajau Exp $
* @package Mambo
* @subpackage Users
...
function userEdit( $option, $uid, $submitvalue) {
        ...
        //these lines are newly added  by sami for gender and country
        //-------------------------------
        $lists=array();
        $lists['gender']             = mosHTML::yesnoRadioList( 'gender', 'class="inputbox" size="1"', $row->gender ,'Male','Female' );
        $lists['country_id']        = mosAdminMenus::CountryList( 'country_id', $row->country_id  );

        HTML_user::userEdit( $row, $option, $submitvalue, $lists);
}


//this is added by sami to change the password

function chanPass( $option, $uid, $submitvalue) {
        global $database;
        if ($uid == 0) {
                mosNotAuth();
                return;
        ...
        HTML_user::userChangePassword ( $row, $option, $submitvalue);
}
...
function userSave( $option, $uid) {
        ...
        {
           if (md5($_POST["passwd"])!= $row->orig_password){
        ...
        if($_POST["pass"]==1)
        {
           mosRedirect ("index.php?option=$option&p=", _USER_PASSWORD_SAVE);
        }
        mosRedirect ("index.php?option=$option&p=", _USER_DETAILS_SAVE);
}
