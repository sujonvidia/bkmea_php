<?php
/**
* @version $Id: admin.member_reg_year_ccci.php,v 1.3 2005/12/14 12:24:00 morshed Exp $
* @package Mambo
...
function editMember_reg_year( $uid=0, $section='' ) {
        ...
        if ($row->checked_out && $row->checked_out <> $my->id) {
                mosRedirect( 'index2.php?option=com_member_reg_year_ccci,The category '. $row->title .' is currently being edited by another administrator' );
        }
...
function saveMember_reg_year( $id ) {
        ...
        */
        ...
        if($id==0){
             ...
             if(mysql_num_rows($res)==0){
                         ...
                         if(!mysql_query($sql_query)) {
                            ...
                            mosRedirect( "index2.php?option=com_member_reg_year_ccci&task=new&mosmsg=".$msg );
                            //echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                            exit();
                         }
                         else{
             ...
             if(mysql_num_rows($res)==0){
                  ...
                  if(!$database->query()) {
                      ...
                      mosRedirect( "index2.php?option=com_member_reg_year_ccci&task=editA&id=$id&mosmsg=".$msg );
                      //echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                      exit();
                  }
             }
        ...
        mosRedirect( "index2.php?option=com_member_reg_year_ccci&mosmsg=".$msg );
}
...
function publishMember_reg_year( $categoryid=null, $cid=null, $publish=1 ) {
        ...
        mosRedirect( 'index2.php?option=com_member_reg_year_ccci');
}
