<?php
/**
* @version $Id: admin.member_type_ccci.php,v 1.1 2005/12/04 04:57:58 aslam Exp $
* @package Mambo
...
function editMember_Type( $uid=0, $section='' ) {
        ...
        if ($row->checked_out && $row->checked_out <> $my->id) {
                mosRedirect( 'index2.php?option=com_member_type_ccci,The category '. $row->title .' is currently being edited by another administrator' );
        }
...
function saveMember_Type( $id ) {
        ...
        */
        ...
        if($id==0){
                ...
                if(mysql_num_rows($res)==0){
                   ...
                   if(!mysql_query($sql_query)) {
                      ...
                      mosRedirect( "index2.php?option=com_member_type_ccci&task=editA&mosmsg=". $msg );
                      //echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                      exit();
                   }
                }else{
                ...
                if(mysql_num_rows($res)==0){
                     ...
                     if(!$database->query()) {
                        ...
                        mosRedirect( "index2.php?option=com_member_type_ccci&task=editA&id=$id&mosmsg=". $msg );
                        //echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                        exit();
                     }
                }
        ...
        mosRedirect( "index2.php?option=com_member_type_ccci&mosmsg=".$msg );
}
...
function publishMember_Type( $categoryid=null, $cid=null, $publish=1 ) {
        ...
        mosRedirect( 'index2.php?option=com_member_type_ccci');
}
