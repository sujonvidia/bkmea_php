<?php
/**
* @version $Id: admin.membership_charge_scci.php,v 1.3 2006/01/01 05:46:59 morshed Exp $
* @package Mambo
...
function showMembership_charge( $section, $option ) {
        ...
        . "\n FROM #__member_charge AS mc , #__member_reg_year AS mry, #__member_type AS mt "
        . "\n ,#__users AS u, #__groups AS g where mc.reg_year_id='".$_SESSION['working_reg_year_id']."' and "
        . "\n mry.id=mc.reg_year_id and  mt.id=mc.member_type_id  "
        . "\n GROUP BY mc.id"
        . $order
...
function editMembership_charge( $uid=0, $section='' ) {
        ...
        if ($row->checked_out && $row->checked_out <> $my->id) {
                mosRedirect( 'index2.php?option=com_membership_charge_scci,The Membership charge id '. $row->id .' is currently being edited by another administrator' );
        }
        /*
        // make order list
        $order = array();
        ...
        */
        ...
        if($id==0){
             ...
             if(mysql_num_rows($res)==0){
                $sql_query="insert into #__member_charge values('','$member_type_id','$reg_year_id','$enrollment_fee','$renewal_fee','$idcard_fee','$certificate_fee','$date','$user_id','$published','','','','','$access','','')";
                $sql_query  =  $database->replaceTablePrefix($sql_query);
                if(!mysql_query($sql_query)) {
                  ...
                  mosRedirect( "index2.php?option=com_membership_charge_scci&task=new&mosmsg=".$msg );
                  exit();
             }
             $msg="Member Charge added successfully";
                  ...
                  $sql_query = "update #__member_charge set
                          ...
                          ,idcard_fee='$idcard_fee', certificate_fee='$certificate_fee'
        ...
        mosRedirect( "index2.php?option=com_membership_charge_scci&mosmsg=".$msg );
}
...
function removeMembership_charge( $section, $cid ) {
        ...
        if (count( $err )) {
                ...
                mosRedirect( 'index2.php?option=com_membership_charge_scci&section='. $section .'&mosmsg='. $msg );
        }
...
function publishMembership_charge( $categoryid=null, $cid=null, $publish=1 ) {
        ...
        mosRedirect( 'index2.php?option=com_membership_charge_scci');
}
...
function accessMenu( $uid, $access, $section ) {
        ...
        mosRedirect( 'index2.php?option=com_membership_charge_scci&section='. $section );
}
...
function menuLink( $id ) {
        ...
        mosRedirect( 'index2.php?option=com_membership_charge_scci&section='. $redirect .'&task=editA&hidemainmenu=1&id='. $id, $msg );
}
...
function saveOrder( &$cid, $section ) {
    ...
    // update ordering values
        ...
        mosRedirect( 'index2.php?option=com_membership_charge_scci', $msg );
} // saveOrder
