...
class HTML_admin_misc {
        ...
        */
        function controlPanel() {
            global $mosConfig_absolute_path, $mainframe, $database, $dbconn;

                 //set working reg year id
                 $sql_query="SELECT id as id, name from #__member_reg_year order by id DESC LIMIT 0, 1";
                 $database->setQuery($sql_query);
                 $res = $database->loadObjectList();
                 $reg_year_id=$res[0]->id;
                 $reg_year_name=$res[0]->name;
                 if(!isset($_SESSION['working_reg_year_id']) ){
                    $_SESSION['max_reg_year_id']=$reg_year_id;
                    $_SESSION['working_reg_year_id']=intval($reg_year_id);
                    $_SESSION['working_reg_year_name']=$reg_year_name;
                 }
        ...
        function system_info( $version ) {
          ...
          <tr>
            <td>
        <strong>For all Camellia functions and features to work ALL of the following directories should be writeable:</strong>
                        <?php
