...
// Blocked by Team Camellia, so that every can access his/her detail and modify
$pd         = trim( mosGetParam( $_REQUEST, 'pd', "") );
//if ($pd==NULL)
//{
//        $pd                        = trim( mosPostParam( $_REQUEST, 'pd', null ) );
//}

if (!($my->usertype=='Super Administrator'||$my->usertype=='Administrator') && $pd==""){
...
function showUsers( $option ) {
        ...
        ;
        if ($search =="" && $filter_type == "0" && $filter_logged =="0") {
               $query = "SELECT COUNT(*)"
               . "\n FROM #__users AS a, #__core_acl_aro_groups as g "
               . "\n where a.gid=g.group_id and g.is_admin=1 "
               ;
        }
        ...
        */
        //
        $query = "SELECT a.*, g.caption AS groupname, s.userid AS loggedin"
        . "\n FROM #__users AS a"
        . "\n INNER JOIN #__core_acl_aro AS aro ON aro.value = a.id"        // map user to aro
        . "\n INNER JOIN #__core_acl_groups_aro_map AS gm ON gm.aro_id = aro.aro_id"        // map aro to group
        . "\n INNER JOIN #__core_acl_aro_groups AS g ON g.group_id = gm.group_id and g.is_admin='1' "
        . "\n LEFT JOIN #__session AS s ON s.userid = a.id"
        . (count( $where ) ? "\n WHERE " . implode( ' AND ', $where ) : "")
        //. "\n and g.is_admin='1' "
        . "\n GROUP BY a.id"
        ...
        $query = "SELECT caption AS value, caption AS text"
...
function editUser( $uid='0', $option='users' ) {
        ...
        */
        $lists['gid']                   = mosAdminMenus::MemberGroup( 'gid', $row->gid );
        // build the html select list
         $gender= $row->gender ? $row->gender : 1 ;

        $lists['gender']             = mosHTML::yesnoRadioList( 'gender', 'class="inputbox" size="1"', $gender ,'Male','Female' );
...
function saveUser( $option, $task ) {
        ...
        if (!$row->store()) {
                //$msg=$row->getError();
                //mosRedirect( "index2.php?option=".$option."&task=".$task."&mosmsg=".$msg );
                echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                exit();
        }
        $row->checkin();
        ...
        switch ( $task ) {
                ...
                default:
                        $msg = 'Successfully Saved User: '. $row->name;
                                                // $pd added by Team Camellia
                                                // mosRedirect( 'index2.php?option=com_users',$msg );
                                                if ($pd=="")
                                mosRedirect( 'index2.php?option=com_users',$msg );
                                                else
                                                        mosRedirect( 'index2.php', 'Your Detail Updated Successfully' );
                        break;
        }
}
