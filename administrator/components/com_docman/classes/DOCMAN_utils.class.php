...
* @version $Id: DOCMAN_utils.class.php,v 1.28 2006/11/22 09:56:05 morshed Exp $
...
class DOCMAN_Utils
{
    function categoryArray()
    {
       ...
       /* $query = "SELECT c.*, c.parent_id AS parent"
         ...
         . "\n AND published <>-2 AND is_delete='0' AND access <= '$_DMUSER->gid'"
         . "\n ORDER BY ordering" ;
         */
         $query = "SELECT c.*, c.parent_id AS parent"
         . "\n FROM #__categories as c "
         . "\n WHERE c.section='com_docman'"
         . "\n AND c.published <>-2 AND c.is_delete='0' AND c.access <= '$_DMUSER->gid'"
         . "\n ORDER BY c.ordering" ;
...
class DOCMAN_Cats
{
    ...
    */
    function getChildsByUserAccess($parent_id = 0, $ordering = "ordering ASC", $userID = null,$upload_year=0)
    {
        ...
        // this query blocked by camellia team
        /*
         ...
         */
         // this query added by camellia team
         // query for finding list of categories order by latest product published date associated of the category
         /* $query="SELECT distinct child.* FROM"
            ."\n (SELECT cat.*,count(cat.id) as no_products,cat.parent_id AS parent_id,MAX(doc.dmdate_published) AS dmdate_published"
            ."\n FROM #__categories AS cat,#__docman AS doc"
            ."\n WHERE cat.id=doc.catid AND cat.is_delete=0 AND LOWER(TRIM(cat.section))='com_docman' AND "
            ."\n cat.published='1' AND doc.is_delete='0' GROUP BY doc.catid) AS child,#__docman AS parent "
            . "\n  WHERE child.id=parent.catid AND parent_id='$parent_id' AND ";
            ;
         */
        if (intval($upload_year)==0){
         $query = "select child.* from (SELECT  c.*,count(c.id) as no_products,"
                               ...
                               . "\n d.dmlink as dmlink,d.catid catid,"
                               ...
                               . "\n WHERE (c.section='com_docman' || c.section='com_trade_lead') and "
                               . "\n  c.published ='1' AND d.published ='1' and c.is_delete='0' and d.is_delete='0'"
                               ."\n and CURDATE()<=d.dmdate_expire and CURDATE()>=d.dmdate_published group by d.catid) as child "
                               . "\n  WHERE child.is_delete='0' AND child.parent_id='$parent_id' AND "
                               ;
        }
        else if (intval($upload_year)==1){
         $query = "select child.* from (SELECT  c.*,count(c.id) as no_products,"
                               ."\n c.section as section,c.checked_out as checked_out_contact_category, "
                               . "\n c.parent_id as parent_id,max(d.dmdate_published) as dmdate_published,max(d.id) as did,"
                               . "\n d.dmlink as dmlink,d.catid as catid,"
                               . "\n d.is_delete as is_delete,d.dmfilename as dmfilename FROM #__categories AS c"
                               . "\n LEFT JOIN #__docman AS d ON d.catid = c.id" //new added
                               . "\n WHERE (c.section='com_docman' || c.section='com_trade_lead') and "
                               . "\n c.published ='1' AND d.published ='1' and c.is_delete='0' and d.is_delete='0'"
                               ."\n and CURDATE()<=d.dmdate_expire and CURDATE()>=d.dmdate_published group by YEAR(d.dmdate_published)) as child "
                               . "\n  WHERE child.is_delete='0' AND child.parent_id='$parent_id' AND "
                               ;
        }
        if($user->userid) {
                if($user->isAdmin) {     //child. added by camellia team
                        $query .= "(child.access='0' OR child.access='1' OR child.access='2')";
                } else {
                        $query .= "(child.access='0' OR child.access='1')";
                }
        } else {
                $query .= "child.access='0'";  //child. added by camellia team
        }
       // $query .= " ORDER BY "; . $ordering; // blocked by camellia team
        $query .= " ORDER BY child.dmdate_published DESC,child.did DESC ";// added by camellia team
        $database->setQuery($query);
    ...
    function countDocsInCatByUser($catid, $user, $include_childs = false)
    {
        ...
        if (!$user->userid/*&& !$_DOCMAN->getCfg('registered')*/) {
            $query .= "\n   WHERE dmowner='" . _DM_PERMIT_EVERYONE . "'"
             . "\n   AND d.published='1' ";
             //. "\n   AND d.approved='1'"; // blocked by camellia team
        } elseif ($user->isAdmin) {
           $query .= " ";
        } elseif ($user->canApprove()) {
                $query .= " ";
        } elseif ($user->canPublish()) {
                 //$query .= "\n WHERE d.approved='1'"; // blocked by camellia team
        } elseif ($user->userid) {
...
class DOCMAN_Docs
{
    ...
    * @param string $ the ordering query
    */
    function getDocsByUserAccess($catid = 0, $ordering = '', $direction = '', $limit = '', $limitstart = 0,$upload_year=0)
    {
        global $database, $_DOCMAN;
        $user = $_DOCMAN->getUser();
        // get ordering
        ...
        // preform query
        $query = "SELECT d.*, c.name AS cat_title FROM #__docman AS d"
         ...
         if (!$user->userid)
         {
            ...
            */
            ...
            $query .= "WHERE d.dmowner='" . _DM_PERMIT_EVERYONE . "'"
                 . "\n AND d.published='1' and CURDATE()<=d.dmdate_expire and CURDATE()>=d.dmdate_published and c.published='1' AND d.is_delete='0'";

                if (intval($upload_year)>0)
                 $query .= "\n AND YEAR(d.dmdate_published)='$upload_year'";
                else
               ...
               {
$query .= " WHERE d.published='1' and CURDATE()<=d.dmdate_expire and CURDATE()>=d.dmdate_published and c.published='1' AND d.is_delete='0'"; // add by camellia team
                if ($user->isAdmin) {
                ...
                $query .= $catid ? "\n AND d.catid IN ('$catid') " : "";
                         ...
                         //$query .= "WHERE d.approved='1'"; // bloceked by camellia team
                ...
                $query .= $catid ? "\n AND d.catid IN ('$catid') " : "";
                    ...
                    //$query .= "WHERE d.published='1' AND d.approved='1'" // blocked by camellia team
                   // $query .= "WHERE d.published='1' and CURDATE()<=d.dmdate_expire and c.published='1' AND d.is_delete='0'" // add by camellia team
      ...
      if (intval($upload_year)>0)
        ...
        $query .= "\n ORDER BY $ordering $direction,d.id DESC ";
        $database->setQuery($query);
    ...
    */
    function search(&$searchArray, $ordering = '', $cats = '', $columns = '', $options = array())
    {
        ...
        if ( empty($options) ) {
            $options = array('search_name', 'search_description','published_date','expire_date');
        }
        ...
        $search_col = array();
        if (is_array($options)) {
            if($searchterms['search_in']!="all"){
               $search_col[] = 'DM.'.$searchterms['search_in'].' ';
            }
            else{
                ...
                // }
                // if (in_array('published_date', $options) ) {
                     $search_col[] = 'DM.dmdate_published ';
                // }
                // if (in_array('expire_date', $options) ) {
                     $search_col[] = 'DM.dmdate_expire ';
                // }
            }
        ...
        $search_in = trim($searchterms['search_in']);
        if (! $text) {
            return array();
        }
        ...
        // modified 'DM.dmlastupdateon DDD' --> 'DM.dmdate_published DDD' by camellia team
        $_DM_SEARCH_SORT_ORDER = array(
            'newest' => 'DM.dmdate_published DDD',
        ...
        $order = "DM.dmdate_published DESC, DM.id DESC";
        /* block by camellia team
        ...
        */
        ...
        $query = "SELECT $list_terms "
         ...
         . "\nWHERE $user_filter "
         //. "\n  AND DM.published='1' AND DM.approved='1'" // blocked by camellia team
         . "\n  AND DM.published='1' AND CURDATE()<=DM.dmdate_expire AND CURDATE()>=DM.dmdate_published and SUB.published='1' AND DM.is_delete='0'" // add by camellia team
         . "\n  AND ($where) "
         . "\nORDER BY $order";
