...
* @version $Id: DOCMAN_file.class.php,v 1.7 2006/04/16 12:19:05 nnabi Exp $
...
class DOCMAN_File
{
    ...
    var $_err                    = null;
        ...
        function DOCMAN_File($name, $path, $catid=Null)   // $catid -- added by camellia team
        {
                //$path = mosPathName( $path );    // blocked by camellia team
                if (!is_dir( $path )) {
                        $path = dirname( $path );
            ...
            $path = mosPathName($path);
                ...
                if( strcasecmp( substr( $this->name , 0, _DM_DOCUMENT_LINK_LNG ) , _DM_DOCUMENT_LINK )==0){
                        ...
                        $this->_isLink = false;
                        //$this->size = filesize($this->path.$this->name); // blocked by camellia team
                        if (file_exists($this->path."/".$catid."/".$this->name) and trim($this->name)!=Null)
                        $this->size = filesize($this->path."/".$catid."/".$this->name); // added by camellia team
    ...
    function readfile_chunked($filename,$retbytes=true)
    {
       ...
       $status = fclose($handle);
         ...
         function exists($catid=Null) {     // parameter $catid added by camellia team
                if( $this->_isLink ){
                        return true;
                }
                if ($catid==Null)
                    ...
                    return file_exists($this->path."/".$catid."/".$this->name);    // added by camellia team
    }
