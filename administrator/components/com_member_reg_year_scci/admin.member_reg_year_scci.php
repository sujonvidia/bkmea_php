<?php
/**
* @version $Id: admin.member_reg_year_scci.php,v 1.5 2006/02/20 11:15:28 morshed Exp $
* @package Mambo
...
function editMember_reg_year( $uid=0, $section='' ) {
        ...
        if ($row->checked_out && $row->checked_out <> $my->id) {
                mosRedirect( 'index2.php?option=com_member_reg_year_scci,The category '. $row->title .' is currently being edited by another administrator' );
        }
        ...
        if($id==0){
             ...
             if(mysql_num_rows($res)==0){
                         ...
                         if(!mysql_query($sql_query)) {
                            ...
                            mosRedirect( "index2.php?option=com_member_reg_year_scci&task=new&mosmsg=".$msg );
                               ...
                               for($i=0; $i<count($rows1); $i++){
                                   ...
                                   for($j=0; $j<count($charge_rows); $j++){
                                        ...
                                        if($charge_row->member_type_id==$type_id ){
                                                ...
                                                $idcard_fee   =$charge_row->idcard_fee;
                                                $certificate_fee   =$charge_row->certificate_fee;
                                                break;
                                        }
                                   }

                                   $insert_query  = "insert into #__member_charge values('','$type_id', "
                                                    ." '".$insert_id."',$enrollment_fee,$renewal_fee,$idcard_fee,$certificate_fee,'$curr_date','$user_id',"
                                                    ." '$published','','','','','$access','','')";
             ...
             $msg="Member registration year added successfully with necessary charge setup";
        }
        else{
             ...
             if(mysql_num_rows($res)==0){
                  ...
                  if(!$database->query()) {
                      ...
                      mosRedirect( "index2.php?option=com_member_reg_year_scci&task=editA&id=$id&mosmsg=".$msg );
                      //echo "<script> alert('".$row->getError()."'); window.history.go(-1); </script>\n";
                      exit();
                  }
             }
             ...
             $msg="Member registration year updated successfully";
        }

        //$row->checkin();
        //$row->updateOrder( "id='$id'" );

        mosRedirect( "index2.php?option=com_member_reg_year_scci&mosmsg=".$msg );
}
...
function removeMember_reg_year( $section, $cid ) {
        ...
        if (count( $cid )) {
                ...
                if (!$database->query()) {
                    ...
                    $msg="Member Registration Year deleted successfully";
        }
...
function cancelMember_reg_year() {
        ...
        //$row->bind( $_POST );
        //$row->checkin();
        mosRedirect( 'index2.php?option=com_member_reg_year_scci' );
}
