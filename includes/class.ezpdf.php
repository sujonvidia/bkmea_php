...
function Cezpdf($paper='a4',$orientation='portrait'){
        ...
        if (!is_array($paper)){
                switch (strtoupper($paper)){
                        ...
                        case 'CUST': {$size = array(0,0,525.00,705.00); break;} //adede by sami for Certificate
                        case 'CUST1': {$size = array(0,0,324.00,324.00); break;} //adede by sami for office copy of Certificate
                        case 'CUSTCCS': {$size = array(0,0,612.45,660.5); break;}//adede by sami for Certificate of sylhet when member type is not associate
                        case 'CUSTASSOCCS': {$size = array(0,0,610.87,637.05); break;}//adede by sami for Certificate of sylhet when member type is associate
                        case 'CUSTCCC': {$size = array(0,0,595.44,662.4); break;}//adede by sami for Certificate of chittagong
...
function ezStartPageNumbers($x,$y,$size,$pos='left',$pattern='{PAGENUM} of {TOTALPAGENUM}',$num=2){
...
function ezPageHeaderReportTitle($x,$y,$size,$pos='center',$pattern='',$num='',$startFromPage=2){
  // put page numbers on the pages from here.
  // place then on the 'pos' side of the coordinates (x,y).
  // pos can be 'left' or 'right'
  // use the given 'pattern' for display, where (PAGENUM} and {TOTALPAGENUM} are replaced
  ...
  $i = count($this->ez['pageNumbering']);
  $this->ez['pageNumbering'][$i][$startFromPage]=array('x'=>$x,'y'=>$y,'pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>$size);
  //$this->ez['pageNumbering'][$i][$startFromPage]=array('x'=>'380','y'=>'818','pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>'8');
  return $i;
}
function ezPageHeaderReportPeriod($x,$y,$size,$pos='center',$pattern='',$num='',$startFromPage=2){
  // put page numbers on the pages from here.
  // place then on the 'pos' side of the coordinates (x,y).
  // pos can be 'left' or 'right'
  // use the given 'pattern' for display, where (PAGENUM} and {TOTALPAGENUM} are replaced
...
function ezTable(&$data,$cols='',$title='',$options='', $fontsize=10){
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.94,0.94,0.94),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>$fontsize,'titleFontSize'=>12
      ...
      foreach($cols as $colName=>$colTitle){
        ...
        if (isset($row[$colName])){
          if (isset($options['cols'][$colName]) && isset($options['cols'][$colName]['link']) && strlen($options['cols'][$colName]['link'])){
            ...
            $lines = explode("\n",$row[$colName]);
            $img_path=$row['photograph'];    //image path
          }
        } else {
          $lines = array();
        }
                           ...
                           if (file_exists($img_path))
                            {
                                ...
                                $this->addJpegFromFile($img_path,$pos[$colName],($this->y)-40,42,45);
                      //$this->addText($pos[$colName],($this->y)-30, 20,$img_path);
                               $image = 111;
                             }
                             else
                                $this->addText( $pos[$colName],$this->y, 10,'No Photo');
                               // $line=$this->addTextWrap($pos[$colName],$this->y,$maxWidth[$colName],$options['fontSize'],'Photo Is Not Available',$just);
