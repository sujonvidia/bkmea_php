...
//pear
require_once( $mosConfig_absolute_path . '/pear/db.php' );
...
class mosHTML {
        ...
        //convert date to insert datebase;
        //$date="16-11-2005";
        //return "2005-11-16";
        ...
        //$date="2005-11-16";
        //return "16-11-2005";
        function ConvertDateDisplayShort($out){
           global $mosconfig_show_date_format_short;
           if(trim($out)=="")
             return "";
           else if(trim($out)=="0000-00-00")
             return "";
           else
             return date($mosconfig_show_date_format_short,strtotime($out));
        }

        function ConvertDateTimeDisplayLong($out){
           global $mosconfig_show_date_time_format;
           if(trim($out)=="")
             return "";
           else if(trim($out)=="0000-00-00")
             return "";
           else
             return date($mosconfig_show_date_time_format,strtotime($out));
        }

        //convert date to display;
        //$date="2005-11-16";
        //return "16th Nov 2005";
        function ConvertDateDisplayLong($date){
           global $mosconfig_show_date_format;
           if ($date=='' || $date=='0000-00-00' || strtolower($date)=='null')
               return '';
           else
               return date($mosconfig_show_date_format,strtotime($date));
        }
  ...
  function writableCell( $folder ) {
        ...
        */
        ...
        function selectList( &$arr, $tag_name, $tag_attribs, $key, $text, $selected=NULL,$section='' ) {
                ...
                if (is_array($arr))
                    reset( $arr );
                /*
                if (strtolower($mosConfig_owner)=='ccci' || strtolower($mosConfig_owner)=='scci'){
                $html = "\n<select name=\"$tag_name\" onChange=\"return belongs_to();\" $tag_attribs>";
                }
                else{
                */
                $html = "\n<select name=\"$tag_name\" $tag_attribs>";
        ...
        * @returns string HTML for the radio list
        */
        function yesnoCheckBox( $tag_name, $tag_attribs, $selected, $javascript, $yes=_CMN_YES, $no=_CMN_NO ) {
                ...
                $html="<input type=checkbox name=$tag_name $box1 value=0 $javascript> ".$no;
                $html.=" <input type=checkbox name=$tag_name $box2 value=1 $javascript> ".$yes;
                return $html;
        }
        ...
        */
        function idBox( $rowNum, $recId, $checkedOut=false, $name='cid' ) {
         ...
         // idBox for delegate voter list
         function idBox_Delegate_voter( $rowNum, $recId, $checked, $checkedOut=false, $name='cid' ) {
                if ( $checkedOut ) {
                        return '';
                } else {
                        return '<input type="checkbox" id="cb'.$rowNum.'" name="'.$name.'[]" value="'.$recId.'" '.$checked.'" onclick="isChecked(this.checked);" />';
                }
        }
...
/**
* Member_reg_year database table class
* @package Mambo
*/
class mosMember_reg_year extends mosDBTable {
        /** @var int Primary key */
        var $id=null;
        /** @var string The full name for the Category*/
        var $name=null;
        ...
        * @param database A database connector object
        */
        function mosMember_reg_year( &$db ) {
                $this->mosDBTable( '#__member_reg_year', 'id', $db );
        }
        // overloaded check function
        function check() {
                // check for valid name
                ...
                // check for existing name
                $this->_db->setQuery( "SELECT id FROM #__member_reg_year "
...
class mosRenew extends mosDBTable {
        /** @var int Primary key */
        var $id=null;
        /** @var string The menu title for the Category (a short name)*/
        var $title=null;
        ...
        * @param database A database connector object
        */
        function mosRenew( &$db ) {
                $this->mosDBTable( '#__member_history', 'id', $db );
        }
        // overloaded check function
        function check() {
                // check for valid name
                if (trim( $this->money_receipt_no ) == '') {
                        $this->_error = "You must have a money receipt no";
                        return false;
                }
                if (trim( $this->amount ) == '') {
                        $this->_error = "You must give a renewal fee amount .";
                        return false;
                }
                // check for existing name
                /*$this->_db->setQuery( "SELECT id FROM #__categories "
...
* @package Mambo
*/
class mosMembership_charge extends mosDBTable {
        /** @var int Primary key */
        var $id=null;
        /** @var string The menu title for the Category (a short name)*/
        var $member_type_id=null;
        /** @var string The full name for the Category*/
        var $reg_year_id=null;
        /** @var boolean */
        var $published=null;
        /** @var boolean */
        var $checked_out=null;
        ...
        * @param database A database connector object
        */
        function mosMembership_charge( &$db ) {
                $this->mosDBTable( '#__member_charge', 'id', $db );
        }
        // overloaded check function
        function check() {
                // check for valid name
                if (trim( $this->member_type_id ) == '') {
                        $this->_error = "Your Category must contain member type.";
                        return true;
                }
                if (trim( $this->reg_year_id ) == '') {
                        $this->_error = "Your Category must have registration year.";
                        return true;
                }
...
* @package Mambo
*/
class mosMember_Type extends mosDBTable {
        /** @var int Primary key */
        var $id=null;
        /** @var string The menu title for the Category (a short name)*/
        var $title=null;
        ...
        * @param database A database connector object
        */
        function mosMember_Type( &$db ) {
                $this->mosDBTable( '#__member_type', 'id', $db );
        }
        // overloaded check function
        function check() {
                // check for valid name
                ...
                // check for existing name
                $this->_db->setQuery( "SELECT id FROM #__member_type "
                . "\nWHERE name='".$this->name
                );
...
* @package Mambo
*/
class mosMember_Category extends mosDBTable {
        /** @var int Primary key */
        var $id=null;
        /** @var string The full name for the Category*/
        var $name=null;
        ...
        * @param database A database connector object
        */
        function mosMember_Category( &$db ) {
                $this->mosDBTable( '#__member_category', 'id', $db );
        }
        // overloaded check function
        function check() {
                if (trim( $this->name ) == '') {
                ...
                // check for existing name
                $this->_db->setQuery( "SELECT id FROM #__member_category "
                . "\nWHERE name='".$this->name
                );
...
class mosUser extends mosDBTable {
        ...
        */
        ...
        function store( $updateNulls=false ) {
                global $acl, $migrate,$database,$my;
                $section_value = 'users';
                ...
                if( $key && !$migrate) {
                        ...
                        $database->setQuery("Insert into #__user_trail values ('','1','".$my->username."','$date','".$this->username."','".$this->name."','".$this->usertype."')"); //to insert the value in user trail table
                                                                                                                                                                                    //1 means insert
                }
                if( !$ret || !$database->query()) {
...
//                $acl->del_object( $aro_id, 'ARO', true );
               $database->setQuery("select username as username, #__users.name as name, caption as usertype from #__users, #__core_acl_aro_groups where #__users.gid=#__core_acl_aro_groups.group_id and #__users.id='".$oid."'");
               $rows = $database->loadObjectList();
               // $caption=$rows[0]->usertype;
                ...
                if ($this->_db->query()) {
                        ...
                        $date= date( "Y-m-d H:i:s" );
                        $database->setQuery("Insert into #__user_trail values ('','2','".$my->username."','$date','".$rows[0]->username."','".$rows[0]->name."','".$rows[0]->usertype."')");   //to insert the value in user trail table
                                                                                                                                                                                               //2 is for delete
                        if (!$this->_db->query()) {
                        ...
                        if (!$this->_db->query() || !$database->query()) {
                                $this->_error = $this->_db->getErrorMsg();
                                return false;
                        }
...
*/
function mosWarning($warning, $title='Camellia Warning') {
        //Default title is changed bt team camellia from 'Mambo Warning'
        global $mosConfig_live_site;
...
class mosAdminMenus {
        ...
        */
        ...
        /**
        * Select list of active Register Year by Morshed
        */
        function RegYear( $name, $active=NULL, $opt='', $javascript=NULL,  $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $RegYear[] = mosHTML::makeOption( '0', "Select Registration Year" );
                        $RegYear = array_merge( $RegYear, $database->loadObjectList() );
                } else {
                        $RegYear = $database->loadObjectList();
                }

                if ( count( $RegYear ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_member_reg_year', 'You must create a Registration Year category first.' );
                }

                $RegYear = mosHTML::selectList( $RegYear, $name, $opt.' class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $RegYear;
        }

         /**
        * Select list of active Group /Associate member
        */
        function GroupAssociateList( $name, $active=NULL, $javascript=NULL, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, firm_name AS text"
                . "\n FROM #__member"
                . "\n WHERE"
                . "\n is_delete = 0 and "
                . "\n ( type_id = 1 || type_id = 2 )"
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $type[] = mosHTML::makeOption( '0', "Select Group/Town Associate " );
                ...
                if ( count( $type ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_member', 'You must enter Member Profile first.' );
                }

                $type = mosHTML::selectList( $type, $name, $opt.' class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );
        ...
        */
        function Designation( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1, $representative=0 ) {
                global $database;

                if ($representative==1)
                {
                         $query = "SELECT id AS value, name AS text"
                        . "\n FROM #__designation where is_representative=1"
                        . "\n ORDER BY name"
                        ;
                }else
                {
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $designation[] = mosHTML::makeOption( '0', "Select Designation" );
                        $designation = array_merge( $designation, $database->loadObjectList() );
                } else {
                        $designation = $database->loadObjectList();
                }

                $designation = mosHTML::selectList( $designation, $name, ' class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );
        ...
        /**
        * Select list of active member type
        */
        /*function MemberType( $name, $section, $active=NULL, $group=0, $javascript=NULL, $opt='', $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                if($group!=0)
                  $sql=" and group_type='$group' ";
                else
                  $sql="";
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $type[] = mosHTML::makeOption( '0', "Select Member Type" );
                        $type = array_merge( $type, $database->loadObjectList() );
                } else {
                        $type = $database->loadObjectList();
                }
                ...
                if ( count( $type ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_member_type', 'You must create a Member Type first.' );
                }
        ...
        function MemberType( $name, $section, $active=NULL, $group=0, $javascript=NULL, $opt='', $order='ordering', $size=1, $sel_cat=1 ) {
                ...
                $type = mosHTML::selectList( $type, $name, $opt.' class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active, $section );

                return $type;
        }

         /**
        * Select list of active member type for report module
        */
        function MemberTypeList( $name, $section, $active=NULL, $group=0,$javascript=NULL, $opt='', $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
        ...
        //Member Group for user management
         function MemberGroup( $name,  $active=NULL, $section=Null, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                                global $my;
                                $gid=$my->gid;
                $query = "SELECT group_id AS value, caption AS text"
                . "\n FROM #__core_acl_aro_groups "
                . "\n WHERE"
                . "\n is_admin = '1' and group_id<=".$gid;
                //. "\n ORDER BY id DESC"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $group[] = mosHTML::makeOption( '0', "Select User Group" );
                        $group = array_merge( $group, $database->loadObjectList() );
                } else {
                ...
                */
                ...
                $group = mosHTML::selectList( $group, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $group;
        }

        //To get country list from pshop_country table by morshed
        function CountryList( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $ctry[] = mosHTML::makeOption( '0', "Select Country" );
                        $ctry = array_merge( $ctry, $database->loadObjectList() );
                } else {
                ...
                $ctry = mosHTML::selectList( $ctry, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );
        ...
        function BankList( $name, $active=1, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, bank_name AS text"
                . "\n FROM #__bank"
                ...
                if ( $sel_cat ) {
                        $bank = array();
                        $bank[] = mosHTML::makeOption( '0', "Select A Bank" );
                        $bank = array_merge( $bank, $database->loadObjectList() );
                } else {
                        $bank = $database->loadObjectList();
                }
               ...
               /* if ( count( $ctry ) < 1 ) {
                ...
                $bank = mosHTML::selectList( $bank, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $bank;
        }

        function MemberTitle( $name, $active=1, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT title AS value, title AS text"
                . "\n FROM #__person_title"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        //$title[] = mosHTML::makeOption( '0', "Select A title" );
                        $title=array();
                        $title = array_merge( $title, $database->loadObjectList() );
                } else {
                        $title = $database->loadObjectList();
                }
               ...
               /* if ( count( $ctry ) < 1 ) {
                ...
                $title = mosHTML::selectList( $title, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $title;
        }


        function ProductUnit( $name, $active=1, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, unit_name AS text"
                . "\n FROM #__production_unit"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $unit[] = mosHTML::makeOption( '0', "Select a Unit" );

                        $unit = array_merge( $unit, $database->loadObjectList() );
                } else {
                        $unit = $database->loadObjectList();
                }



                $unit = mosHTML::selectList( $unit, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $unit;
        }

        function Machine( $name, $active=1, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, machine_type AS text"
                . "\n FROM #__machine"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $machine[] = mosHTML::makeOption( '0', "Select a machine" );

                        $machine = array_merge( $machine, $database->loadObjectList() );
                } else {
                        $machine = $database->loadObjectList();
                }



                $machine = mosHTML::selectList( $machine, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $machine;
        }

        //prepare payment mode radio list by Morshed
        function PaymentMode( $name, $active=NULL, $javascript=NULL ) {
                global $database;

                $pmode=array();
                $pmode['Cash']='Cash';
                $pmode['Checque']='Cheque';
                $pmode['P. O']='P. O';
                $pmode['D. D']='D. D';

                foreach($pmode as $key=>$val){
                        if($active==$val)
                           $html.="<input type='radio' name='".$name."' value='".$val."' checked> $val";
                        else
                           $html.="<input type='radio' name='".$name."' value='".$val."' > $val";
                        $html.="&nbsp;&nbsp;&nbsp;";
                }
        ...
        * Select list of active categories for components
        */
        function ProductCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
               ...
               $query = "SELECT id AS value, name AS text"
                . "\n FROM #__categories c"
                //. "\n WHERE section='com_docman' AND "
                . "\n WHERE "
                . "\n published ='1' AND is_delete ='0'"
                . "\n ORDER BY ordering" ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $categories[] = mosHTML::makeOption( '0', 'All Categories' );
                        $categories = array_merge( $categories, $database->loadObjectList() );
                } else {
                        $categories = $database->loadObjectList();
                }
        ...
        /**
        * Select list of active info products
        */
        function infoProductList( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;



               // get a list of the menu items
               $query = "SELECT id AS value, dmname AS text"
                . "\n FROM #__docman as d"
                . "\n WHERE published ='1' AND is_delete ='0'"
                . "\n ORDER BY dmdate_published" ;

                $database->setQuery( $query );

                if ( $sel_cat ) {
                        if(trim(strtolower($mosConfig_owner))!="epb"){
                           $infoProduct[] = mosHTML::makeOption( '0', 'All Trade Leads' );
                        }
                        else{
                           $infoProduct[] = mosHTML::makeOption( '0', 'All Info Products' );
                        }
                        $infoProduct = array_merge( $infoProduct, $database->loadObjectList() );
                } else {
                        $infoProduct = $database->loadObjectList();
                }
                /*
                if ( count( $infoProduct ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_categories&section='. $section, 'You must create a category first.' );
                }
               */
                $ProductList = mosHTML::selectList( $infoProduct, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $ProductList;
        }
        //
        function SearchLocation( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, location AS text"
                . "\n FROM #__member_location order by location"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $location[] = mosHTML::makeOption( 'all', "All" );
                        $location = array_merge( $location, $database->loadObjectList() );
                } else {
                        $location = $database->loadObjectList();
                }

                if ( count( $location ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_country', 'You must create Location List first.' );
                }

                $location = mosHTML::selectList( $location, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $location;
        }

        //
        function MemberTin( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database, $config_voter_election_date,$mosconfig_voter_new_date,$mosconfig_voter_renew_date;

                $election_date=mosHTML::ConvertDateForDatatbase($config_voter_election_date);
               // $query = "SELECT distinct(tin) AS value, tin AS text"
                //. "\n FROM #__member order by tin"
                //;
                $query="select tin as value,tin as text ,count(*) as count1 from mos_member as m4"
                ."\n left join #__member_history as mh1 on m4.id=mh1.member_id"
                ."\n where m4.is_delete=0 and mh1.reg_year_id= '".$_SESSION['working_reg_year_id']."' and"
                ."\n ((mh1.entry_type=1 and mh1.reg_date<= date_sub('$election_date', interval '$mosconfig_voter_new_date' day)) || "
                ."\n (mh1.entry_type=2 and mh1.reg_date<= date_sub('$election_date', interval '$mosconfig_voter_renew_date' day)))"
                ."\n  and m4.tin in (select distinct tin as tin from mos_member) group by m4.tin having count1>1";

                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $tin[] = mosHTML::makeOption( '0', "Select Tin Number" );
                        $tin = array_merge( $tin, $database->loadObjectList() );
                } else {
                        $tin = $database->loadObjectList();
                }

                $tin = mosHTML::selectList( $tin, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $tin;
        }

        //prepare search list for public site by Morshed
        function SearchOption( $name, $active, $section=Null, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database, $mosConfig_owner;
                $arr["all"]      ="All Type";
                if(trim(strtolower($mosConfig_owner))=="bkmea"){
                   $arr["member_reg_no"]  ="Member Reg No.";
                }
                $arr["company"]  ="Company Name";
                $arr["person"]   ="Person Name";
                $arr["road"]     ="Street/ Area";
                $arr["date"]     ="Date Updated";
                $arr["size1"]    ="Employee Size >=";
                $arr["size2"]    ="Employee Size <=";
                $arr["capacity1"]="Production Capacity >=";
                $arr["capacity2"]="Production Capacity <=";
                //additional search type for bkmea
                if(trim(strtolower($mosConfig_owner))=="bkmea"){
                   $arr["total_no_machine1"]="Total No. of Machine >=";
                   $arr["total_no_machine2"]="Total No. of Machine <=";
                   $arr["investment_amount"]="Investment amount";
                }
                //additional search type for scci
                if(trim(strtolower($mosConfig_owner))=="scci" || trim(strtolower($mosConfig_owner))=="ccci"){
                   $arr["turn_over_volume1"]="Turn over volume >=";
                   $arr["turn_over_volume2"]="Turn over volume <=";
                   $arr["member_reg_no"]="Membership Number";
                }
                $sel=0;
                ...
                $html="<select name='".$name."' class=inputbox>".$html;
        ...
        //prepare search list for public site by Morshed
        function InfoProductSearchOption( $name, $active, $section=Null, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database, $mosConfig_owner;
                $arr["all"]     ="All Attributes";
                $arr["dmname"] ="Info Product Name";
                $arr["dmdescription"]  ="Abstract";
                $arr["dmdate_published"]    ="Publication Date";
                $arr["dmdate_expire"]    ="Expire Date";
                $sel=0;
        ...
        //
        function Location( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $location[] = mosHTML::makeOption( '0', "Select Location" );
                        $location = array_merge( $location, $database->loadObjectList() );
                } else {
                        $location = $database->loadObjectList();
                }
        ...
        /*
        * prepare corporate Status list
        */
       /* function CorporateStatus( $name, $active, $section, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                $arr["Public Limited"]="Public Limited";
                $arr["Private Limited"]="Private Limited";
                $arr["Partnership"]="Partnership";
                $arr["Proprietorship"]="Proprietorship";
                $arr["Other"]="Other";
                ...
                $html.="</select>";
                if($sel==0)
                  $html="<option value='0' selected> Select Corporate Status </option>".$html;
        ...
        function CorporateStatus( $name, $active, $section='', $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                ...
                if(trim(strtolower($mosConfig_owner))=="ccci")
                {
                        $arr["Limited Company"]="Limited Company";
                }
                else {
                ...
                $sel=0;
                $html="<option value='0'> Select Corporate Status </option>";
                foreach ($arr as $k => $v){
                        if($active==$k){
                          $html.="<option value='".$v."' selected>".$k."</option>";
                          $sel=1;
                ...
                //if($sel==0)
                  ...
                  if (trim($section)=='ccci')
                  $html="<select name=\"$name\" onChange=\"return corporate();\" class=\"inputbox\">".$html;
                  else
                  $html="<select name=\"$name\" class=\"inputbox\">".$html;
        ...
        // Select list of user type for report module
        function UserTypeList( $name, $active, $type='u', $javascript=NULL ) {
                global $database,$option;

                $arr=array();
                /*$arr[0]="All";
                $arr[1]="Super Administrator";
                $arr[2]="Administrator";
                $arr[3]="Manager";
                $arr[4]="Registered User";
                  */
                $arr[0]="All";
                $arr[1]="Biz Admin";
                $arr[2]="Sys Admin";
        ...
        * User report for
        */
       function UserReportFor( $name, $active, $type='m', $javascript=NULL ) {
                global $database;
          ...
          /**
        * Select list of administrative users for member history report module
        */
        function MemberHistoryTypeList( $name, $section, $active=NULL, $javascript=NULL, $opt='', $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "select u.username as value, u.name as text from #__users as u"
                         ."\n JOIN #__core_acl_aro_groups as g ON u.gid=g.group_id"
                         . "\n where g.is_admin=1"
                         //. "\n ORDER BY id DESC"
                         ;
                $database->setQuery( $query );
                ...
                return $type;
                        ...
                        /**
        * Members History report for
        */
       function MemberHistoryReportFor( $name, $active, $type='m', $javascript=NULL ) {
                global $database,$mosConfig_db, $for;

                $arr=array();
                $arr[0]="All";
                $arr[1]="New Profile";
                $arr[2]="Renew Member";
                $arr[3]="Update Profile";
                $arr[4]="Member Certificate";
                if ($for=='scci')
                   $arr[5]="Member Id Card";
                $arr[6]="Delete Profile";
        ...
        /*
        * List to select Report for of Account Transaction Report;
        */
       function AccountsTransactionFor( $name, $section, $active=NULL, $javascript=NULL, $opt='', $order='ordering', $size=1, $sel_cat=1  ) {
                global $database;

                 $query = "SELECT id AS value, name AS text"
                . "\n FROM #__account_chart"
                . "\n where published = 1 order by id"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $accounts_transaction[] = mosHTML::makeOption( '0', "All" );
                        $accounts_transaction = array_merge( $accounts_transaction, $database->loadObjectList() );
                } else {
                        $accounts_transaction = $database->loadObjectList();
                }
                $accounts_transaction[] = mosHTML::makeOption( '-99', "Summary" );
                //array_push($accounts_transaction,'Summery');
                /*if ( count( $type ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_member_type', 'You must create a Member Type first.' );
                }*/

                $accounts_transaction = mosHTML::selectList( $accounts_transaction, $name, $opt.' class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $accounts_transaction;
        }
        /*
        * List to select Report for;
        * var $type m for Member and v for voter
        */
        function ReportFor( $name, $active, $opt=Null, $for=0, $javascript=NULL ) {
                global $database;
                ...
                if($for==0){
                     $arr["0"]="All";
                     $arr["1"]="New Member";
                     $arr["2"]="Renewed Member";
                }
                if($for==1)
                  $arr["3"]="Outstanding Member";
                ...
                $html="<select $opt name='".$name."' class=inputbox>".$html;
        ...
        */
        function MultiCountryList( $name, $active) {
                global $database;
                $query = "SELECT country_id AS value, country_name AS text"
                . "\n FROM #__pshop_country"
                . "\n where published = 1"
                ...
                $clist = mosHTML::multipleselectList( $clist, $name, 'class=inputbox size=3 multiple', 'value', 'text', $_SESSION[$active] );
        ...
        * Create Multiple select Country List
        */
        function MultipleBusinessType( $name, $active) {
                global $database;
                $query = "SELECT id AS value, name AS text"
                . "\n FROM #__business_type_info"
                ;
                $database->setQuery( $query );
                $businesslist = $database->loadObjectList();
                $businesslist = mosHTML::multipleselectList( $businesslist, $name, 'class=inputbox size=3 multiple', 'value', 'text', $_SESSION[$active] );
        ...
        function MultipleAssociationList( $name, $active) {
               ...
               if ( count( $al ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_association_list', 'You must create Association List first.' );
                }

                $al = mosHTML::multipleselectList( $al, $name, 'class=inputbox size=3 multiple', 'value', 'text', $_SESSION[$active] );

                return $al;
        }

        function MultipleProductList( $name, $active) {
                global $database;
                ...
                /*if ( count( $pl ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_product_list', 'You must create Product List first.' );
                } */
                $pl = mosHTML::multipleselectList( $pl, $name, 'class=inputbox size=3 multiple', 'value', 'text', $_SESSION[$active] );

                return $pl;
        }

        //active product list for single select by Morshed
        function ProductList( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, concat(hscode,', ',name) AS text"
                . "\n FROM #__product_line WHERE parent_id>0"
                ;
                $database->setQuery( $query );
                //$pl = $database->loadObjectList();
                $pl = array();
                if ( $sel_cat ) {
                        $pl[] = mosHTML::makeOption( 'all', "All" );
        ...
        function ComplianceList( $name, $active) {
                global $database;

                $query = "SELECT id AS value, name AS text"
                . "\n FROM #__compliance_list"
                ;
                $database->setQuery( $query );
                $cl = $database->loadObjectList();
                /*if ( count( $cl ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_product_list', 'You must create Product List first.' );
                }*/
                $cl = mosHTML::multipleselectList( $cl, $name, 'class=inputbox size=5 multiple="multiple" ', 'value', 'text', $_SESSION[$active] );

                return $cl;
        }


        function ComplianceListSingle( $name, $active=NULL, $sel_cat=1) {
                global $database;
                ...
                ;
                $database->setQuery( $query );
                $cl[] = mosHTML::makeOption( '0', "Select Certificate" );
                $cl   = array_merge( $cl, $database->loadObjectList() );

                $cl = mosHTML::selectList( $cl, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $cl;
        }

        function Association( $name, $active) {
                global $database;
        ...
        function ExportCategory( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT id AS value, cat_no AS text"
                . "\n FROM #__export_cat_list"
                . "\n ORDER BY id DESC"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $export_cat[] = mosHTML::makeOption( '0', "Ex. Category " );
                        $export_cat = array_merge( $export_cat, $database->loadObjectList() );
                } else {
                        $export_cat = $database->loadObjectList();
                }

                if ( count( $export_cat ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_export_category', 'You must create Export Category first.' );
                }

                $export_cat = mosHTML::selectList( $export_cat, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $export_cat;
        }

        function ProposerList( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;

                $query = "SELECT member_reg_no AS value, firm_name AS text"
                . "\n FROM #__member"
                . "\n ORDER BY id DESC"
                ;
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $proposer[] = mosHTML::makeOption( '0', "Select Proposer" );
                        $proposer   = array_merge( $proposer, $database->loadObjectList() );
                } else {
                        $proposer = $database->loadObjectList();
                }

                if ( count( $proposer ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_membership_bkmea', 'You must submit member information first.' );
                }

                $proposer = mosHTML::selectList( $proposer, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );
        ...
        */
        function MemberCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                ...
                /*
                if ( count( $categories ) < 1 ) {
                        mosRedirect( 'index2.php?option=com_member_category', 'You must create a Member Category first.' );
                }
                */
        ...
        function BusinessType( $name, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                ...
                ;
                $database->setQuery( $query );
                $businesslist=array(mosHTML::makeOption( '0', "All" ));
                //$businesslist[] = mosHTML::makeOption( '0', "All" );
                if(count($database->loadObjectList())>0)
                   $businesslist = array_merge( $businesslist, $database->loadObjectList() );
                $businesslist = mosHTML::selectList( $businesslist, $name, 'class="inputbox" size="'. $size .'" '. $javascript, 'value', 'text', $active );

                return $businesslist;
        }

        function ReportMemberCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
                global $database;
                ...
                $database->setQuery( $query );
                if ( $sel_cat ) {
                        $categories[] = mosHTML::makeOption( '0', "All" );
                        $categories = array_merge( $categories, $database->loadObjectList() );
                } else {
                        $categories = $database->loadObjectList();
                }
...
class mosCommonHTML {
        ...
        */
        ...
        // checked lists for delegated voter if is voter
        function CheckedOutProcessing_Delegate_voter( &$row, $i ) {
                global $my;

                if ( $row->checked_out ) {
                        $checked = mosCommonHTML::checkedOut( $row );
                } elseif ($row->is_voter==1) {
                        $checked = mosHTML::idBox_Delegate_voter( $i, $row->id, "checked=true" );
                }
                else {
                        $checked = mosHTML::idBox_Delegate_voter( $i, $row->id," "  );
                }
...
class mosSearchPublicSite{
      var $query=null;
      var $invoice_id=0;

      function mosSearchPublicSite(){
              $query=null;
              $invoice_id=0;
      }
      ...
      function saveSearchQuery(){
               global $database;
               $sql="update #__invoice set file_name=\"".$this->query."\" where id='".$this->invoice_id."'";
               $database->setQuery($sql);
               return $database->query();
      }

      function execSearchQuery(){
              global $database;
              $sql="select file_name from #__invoice where id='".$this->invoice_id."'";
              $database->setQuery( $sql );
              $this->query=$database->loadResult();
              $database->setQuery( $this->query );
              $list = $database->loadObjectList();
              return $list;
      }

      function botSearchMemberBkmea( $report_type, $phrase, $text, $search_type='all', $product='all', $is_direct_export='all', $location='all', $member_category_id='all', $compliance_id='all') {
        global $my, $database;
        ...
        */
                        ...
                        case 'exact':
                        {
                                ...
                                switch($search_type){
                                        case 'member_reg_no':
                                                ...
                                                if ($membership_no!=""){
                                                  ...
                                                  if (sizeof($members2)>0)
                                                      $wheres2[] = "m.member_reg_no IN ($members)";
                                               }
                                              break;
                                        case 'company':
                                              ...
                                              break;
                                        case 'person':
                                              $wheres2[] = "LOWER(m.applicant_name) LIKE '%$text%' or LOWER(m.applicant_last_name) LIKE '%$text%'";
                                              break;
                                        case 'road':
                                              $wheres2[] = "LOWER(m.applicant_address_street) LIKE '%$text%'";
                                              break;
                                        case 'date':
                                              $wheres2[] = "LOWER(m.date)='$text'";
                                              break;
                                        case 'size1':
                                              if(is_numeric($text))
                                        ...
                                        case 'investment_amount':
                                              if(is_numeric($text))
                                                 $wheres2[] = "LOWER(m.investment) = '$text'";
                                              break;
                                        case 'all':
                                        default:
                                              $wheres2[] = "LOWER(m.firm_name) LIKE '%$text%'";
                                         ...
                                         switch($search_type){
                                               case 'member_reg_no':
                                                 ...
                                                 case 'company':
                                                       ...
                                                       break;
                                                 case 'person':
                                                       $wheres2[] = "LOWER(m.applicant_name) LIKE '%$word%'  or LOWER(m.applicant_last_name) LIKE '%$word%'";
                                                       break;
                                                 case 'road':
                                                       $wheres2[] = "LOWER(m.applicant_address_street) LIKE '%$word%'";
                                                       break;
                                                 case 'date':
                                                       $wheres2[] = "LOWER(m.date)='$word'";
                                                       break;
                                                 case 'size1':
                                                       if(is_numeric($word))
                                                 ...
                                                 case 'investment_amount':
                                                       if(is_numeric($word))
                                                          $wheres2[] = "LOWER(m.investment) ='$word'";
                                                       break;
                                                 case 'all':
                                                 default:
                                                       $wheres2[] = "LOWER(m.firm_name) LIKE '%$word%'";
                ...
                $direct_export_search="";
                if($is_direct_export!='all' && is_numeric($is_direct_export) ) {
                     $direct_export_search = " and ( m.is_direct_export='$is_direct_export' )";
                }
                else
                    $direct_export_search="";
                ...
                $member_category_search="";
                if($member_category_id!="all" && is_numeric($member_category_id) ) {
                     $member_category_search = " and ( m.member_category_id='$member_category_id' )";
                }
                else
                    $member_category_search="";

                $compliance_search="";
                if($compliance_id!="all" && is_numeric($compliance_id) ) {
                     $compliance_search = " and ( mcl.compliance_id='$compliance_id' )";
                }
                else
                    $compliance_search="";

                if($report_type=="statistics"){
                     $sql = "SELECT count(distinct(m.id)) as total_member,m.country_id as country_id"
                       . "\n FROM #__member AS m"
                       . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                       . "\n Left Join #__member_compliance_list as mcl on mcl.member_id = m.id"
                       . "\n WHERE m.is_delete='0' and ( $where ) $product_search $location_search "
                       . "\n $member_category_search $compliance_search $direct_export_search "
                ...
                if($report_type=="general"){
                   $sql = "SELECT distinct( m.id ) AS id, m.firm_name AS firm_name"
                   . "\n , m.applicant_title as title, m.applicant_name as firstname, m.applicant_last_name as lastname "
                   . "\n , m.applicant_address_street as street, m.applicant_address_town_suburb as town "
                   . "\n , m.applicant_address_district as district, m.applicant_address_division as division "
                   . "\n , m.applicant_address_country as country "
                   . "\n , m.employee_total as employee "
                   . "\n , m.production_capacity as capacity, m.date as date "
                   . "\n , ml.location as location "
                   . "\n FROM #__member AS m"
                   . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                   . "\n Left Join #__member_compliance_list as mcl on mcl.member_id = m.id"
                   . "\n Left Join #__member_location as ml on ml.id = m.location"
                   . "\n WHERE m.is_delete='0' and ( $where ) $product_search $location_search $direct_export_search "
                   . "\n $member_category_search $compliance_search "
                   ;
                   $this->query=$sql;
                   $database->setQuery( $sql );
                   $list = $database->loadObjectList();
                ...
                if($report_type=="detail"){
                   $sql = "SELECT m.id as id, m.firm_name as firm_name, m.commencing_date as commencing_date"
                   . "\n ,mt.name as type, ml.location as location"  //, mry.name as reg_year
                   . "\n , m.applicant_title as title, m.applicant_name as firstname, m.applicant_last_name as lastname "
                   . "\n , m.applicant_address_street as street"
                   ...
                   . "\n ,m.employee_male as male,m.employee_female as female,m.employee_total as total_employee"
                   . "\n ,m.date as date, m.last_reg_date as reg_date,m.production_capacity as capacity"
                   . "\n ,m.office_address_street as fstreet, m.office_address_town_suburb as ftown, "
                   . "\n m.office_address_district as fdistrict"
                   . "\n ,m.office_address_division as fdivision, m.office_address_country as fcountry"
                   . "\n FROM #__member AS m"
                   . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                   . "\n Left Join #__member_compliance_list as mcl on mcl.member_id = m.id"
                   . "\n Left Join #__member_location as ml on ml.id = m.location"
      ...
      function botSearchMemberScci( $report_type, $phrase, $text, $search_type='all', $product='all', $location='all', $corporate_status='all', $business_type='all', $country_id='all') {
        global $my, $database;
        global $mosConfig_abolute_path, $mosConfig_offset;
        ...
        */
                        ...
                        case 'exact':
                        {
                                ...
                                switch($search_type){
                                        ...
                                        case 'person':
                                              ...
                                              break;
                                        case 'road':
                                              $wheres2[] = "LOWER(m.firm_reg_address_street) LIKE '%$text%'";
                                              break;
                                        case 'date':
                                              $wheres2[] = "LOWER(m.date)='$text'";
                                              break;
                                        ...
                                        case 'size2':
                                              if(is_numeric($text))
                                                $wheres2[] = "m.employee_total<='$text'";
                                              break;
                                        case 'turn_over_volume1':
                                              if(is_numeric($text))
                                                $wheres2[] = "m.financial_turnover_volume>='$text'";
                                              break;
                                        case 'turn_over_volume2':
                                              if(is_numeric($text))
                                                $wheres2[] = "m.financial_turnover_volume<='$text'";
                                              break;
                                        case 'member_reg_no':
                                              {
                                                   $members1=explode(",",$text);
                                                   $members2=array();
                                                   for ($i=0;$i<count($members1);$i++)
                                                   {
                                                        if(trim($members1)!="")
                                        ...
                                        default:
                                              ...
                                              if(is_numeric($text))
                                                 $wheres2[] = "m.financial_turnover_volume='$text'";
                                              if(is_numeric($text))
                                                 $wheres2[] = "m.member_reg_no LIKE '%$text%'";
                                              break;
                                        ...
                                        $wheres2 = array();
                                         switch($search_type){
                                                 ...
                                                 case 'person':
                                                       ...
                                                       break;
                                                 case 'road':
                                                       $wheres2[] = "LOWER(m.firm_reg_address_street) LIKE '%$word%'";
                                                       break;
                                                 case 'date':
                                                       $wheres2[] = "LOWER(m.date)='$word'";
                                                       break;
                                                 ...
                                                 case 'turn_over_volume1':
                                                       if(is_numeric($word))
                                                         $wheres2[] = "m.financial_turnover_volume>='$word'";
                                                       break;
                                                 case 'turn_over_volume2':
                                                       if(is_numeric($word))
                                                         $wheres2[] = "m.financial_turnover_volume<='$word'";
                                                 ...
                                                 default:
                                                       ...
                                                       if(is_numeric($word))
                                                         $wheres2[] = "m.financial_turnover_volume='$word'";
                                                       if(is_numeric($word))
                                                         $wheres2[] = "m.member_reg_no LIKE '%$word%'";
                                                       break;
                ...
                $corporate_status_search="";
                if($corporate_status!="all" ) {
                     $corporate_status_search = " and ( m.corporate_status='$corporate_status' )";
                }
                else
                    $corporate_status_search="";

                $business_type_search="";
                if($business_type!="all" && is_numeric($business_type) ) {
                     $business_type_search = " and ( mbt.business_type='$business_type' and mbt.member_id=m.id )";
                }
                else
                    $business_type_search="";

                $country_search="";
                if($country_id!="all" && is_numeric($country_id) ) {
                   if($country_id==18)
                      $country_search = " and ( m.country_id='$country_id' || m.country_id='0' || m.country_id='')";
                   else
                      $country_search = " and ( m.country_id='$country_id' )";
                }
                else
                    $country_search="";
                ...
                if($report_type=="statistics"){
                   $sql = "SELECT count(distinct(m.id)) as total_member,m.country_id as country_id"
                       . "\n FROM #__member AS m"
                       . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                       . "\n Left Join #__member_business_type as mbt on mbt.member_id = m.id"
                       . "\n WHERE m.is_delete='0' and ( $where ) $product_search $location_search "
                       . "\n and m.id not in (select old_member_id as id from #__expire_member_maping) "
                       . "\n $corporate_status_search $country_search $business_type_search"
                       . "\n group by m.country_id"
                       ;
                ...
                if($report_type=="general"){
                   $sql = "SELECT distinct( m.id ) AS id, m.firm_name AS firm_name"
                       ...
                       . "\n , m.firm_reg_address_country as country "
                       . "\n , m.employee_total as employee "
                       . "\n , m.production_capacity as capacity, m.date as date "
                       . "\n , ml.location as location "
                       . "\n FROM #__member AS m"
                ...
                if($report_type=="detail"){
                   $sql = "SELECT distinct( m.id ) AS id, m.firm_name AS firm_name, m.member_reg_no AS reg_no"
                       . "\n , m.applicant_title as title, m.applicant_name as first_name , m.applicant_last_name as last_name "
                       . "\n , m.firm_reg_address_street as street, m.firm_reg_address_town_suburb as town "
                       . "\n , m.firm_reg_address_district as district, m.firm_reg_address_division as division "
                       . "\n , m.firm_reg_address_country as country "
                       . "\n , m.head_office_address_street as hstreet, m.head_office_address_town_suburb as htown "
                       . "\n , m.head_office_address_district as hdistrict, m.head_office_address_division as hdivision "
                       . "\n , m.head_office_address_country as hcountry "
                       . "\n , m.employee_male as male,m.employee_female as female,m.employee_total as total_employee"
                       . "\n , m.date as date, m.last_reg_date as reg_date,m.production_capacity as capacity"
                       . "\n , m.production_capacity as capacity, m.date as date "
                       . "\n , mt.name as type_name, ml.location as location "
                       . "\n , m.firm_phone as firm_phone, m.firm_mobile as firm_mobile"
                       . "\n , m.firm_fax as firm_fax, m.firm_email as firm_email"
                       . "\n , m.head_office_phone as head_office_phone, m.head_office_mobile as head_office_mobile"
                       . "\n , m.head_office_fax as head_office_fax, m.head_office_email as head_office_email"
                       . "\n FROM #__member AS m"
                       . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                       . "\n Left Join #__member_location as ml on ml.id = m.location"
                       . "\n Left Join #__member_type as mt on mt.id = m.type_id"
      ...
      function botSearchMemberCcci( $report_type, $phrase, $text, $search_type='all', $product='all', $location='all', $corporate_status='all', $is_outside_city='all', $country_id='all', $business_type='all') {
        global $my, $database;
        global $mosConfig_abolute_path, $mosConfig_offset;
        ...
        */
                ...
                $is_outside_city_search="";
                if($is_outside_city!=="all"  ) {
                     $is_outside_city_search = " and ( m.is_outside='$is_outside_city' )";
                }
                else
                    $is_outside_city_search="";
                ...
                if($report_type=="statistics"){
                  $sql = "SELECT count(distinct(m.id)) as total_member,m.country_id as country_id"
                       ...
                       . "\n Left Join #__member_product_line as mpl on mpl.member_id = m.id"
                       . "\n Left Join #__member_business_type as mbt on mbt.member_id = m.id"
                       . "\n WHERE m.is_delete='0' and ( $where ) $product_search $location_search $is_outside_city_search "
                       . "\n and m.id not in (select old_member_id as id from #__expire_member_maping) "
                       . "\n $corporate_status_search $country_search $business_type_search"
                       . "\n group by m.country_id"
                       ;
                ...
                if($report_type=="general"){
                   $sql = "SELECT distinct( m.id ) AS id, m.firm_name AS firm_name"
                       ...
                       . "\n Left Join #__member_location as ml on ml.id = m.location"
                       . "\n Left Join #__member_business_type as mbt on mbt.member_id = m.id"
                       . "\n WHERE m.is_delete='0' and ( $where ) $product_search $location_search $business_type_search"
                       . "\n and m.id not in (select old_member_id as id from #__expire_member_maping) "
                       . "\n $corporate_status_search $country_search $is_outside_city_search"
                       ;
                       $this->query=$sql;
                       $database->setQuery( $sql );
                       $list = $database->loadObjectList();
