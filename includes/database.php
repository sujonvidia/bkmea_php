...
class database {
        ...
        */
        ...
        //insert member trail
        function addMemberTrail($member_id=NULL, $entry_type=NULL, $user_id=NULL, $comments=NULL, $bank_id=NULL, $money_receipt_no=NULL) {
                $current_date = date( "Y-m-d H:i:s" );
                if($member_id==NULL || $entry_type==NULL || $user_id==NULL )
                     return FALSE;
                $query="insert into #__member_trail values('','".$member_id."','".$entry_type."','".$comments."','".$current_date."','".$user_id."','".$bank_id."','".$money_receipt_no."')";
                $query= $this->setQuery($query);
                if($this->query())
                     return true;
        }
        //return member type name
        function getMemberTypeName($type_id=0 ) {
                if(intval($type_id)==0)
                   return "";
                $query="select name as type_name from #__member_type where id='".$type_id."'";
                $this->setQuery($query);
        ...
        */
        ...
        function replaceTablePrefix( $sql, $prefix='#__' ){
                global $mosConfig_dbprefix;
                $sql=str_replace("#__",$mosConfig_dbprefix,$sql);
                return $sql;
        }
