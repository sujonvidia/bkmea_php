...
function showPathway( $Itemid ) {
        ...
        $menu="<span class=\"pathway\">". $home ." ". $path ."</span>\n";

        //menu for membership
        if($option=="com_membership"){
                global $Itemid,$type_id,$id,$task;

                $home = '<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">'. $home .'</a>';
                $img = '<img src="' . $mosConfig_live_site . '/images/M_images/arrow.png" alt="arrow" />';
                $member=$img. "<A href='index.php?option=com_membership&task=category&Itemid=$Itemid' class=pathway>Membership</a>";
                if(isset($id) && $task=="view"){
                        $list=$img. "<A href='index.php?option=com_membership&Itemid=$Itemid&type_id=$type_id' class=pathway>Member List</a>";
                        $menu="$home $member $list $img Detail";
                }
                if(isset($catid) && $task!="view"){
                        $category=$img."<A href='index.php?option=com_membership&Itemid=$Itemid' class=pathway'>Type</a>";
                        $menu="$home $member $img Member List";

                }
                if($task=="category"){
                        $menu="$home $img Membership";
                }
                //$menu="$home $member $category $list ";

        }

        global $mosConfig_owner;
        if(strtolower($option)=="com_directory_".strtolower($mosConfig_owner)){
               $img = '<img src="' . $mosConfig_live_site . '/images/M_images/arrow.png" alt="arrow" />';
               $menu = '<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">'. $home .'</a>'.$img;
               $menu.='<a href="index.php?option=com_search_'.strtolower($mosConfig_owner).'&Itemid=5" class="pathway">Advanced Search</a>'.$img;
        ...
        if($option=="com_docman"){
                ...
                $home = '<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">'. $home .'</a>';
                $img = '<img src="' . $mosConfig_live_site . '/images/M_images/arrow.png" alt="arrow" />';
                if(trim($task)=="" && trim($task)=="cat_view" && intval($gid)!=0){
                   $menu=$home.$img.'<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">Categories</a>'.$img.'Info Product';
                }
                else if(trim($task)=="" && trim($task)=="cat_view"){
                   $menu=$home.$img.'Categories';
                }
                if(isset($Itemid)){
                        $menu.=$img.'<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">Categories</a>'.$sub ;
                }
        } */
        echo $menu;
}
